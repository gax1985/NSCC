...
var ChatCbt = class {
  ...
  async chat({ apiKey, messages, isSummary = false, mode = "openai", ollamaUrl }) {
  ...
  async _openai_chat(messages, apiKey) {
  ...
  async _ollama_chat(messages, ollamaUrl) {
...
var ChatCbtPlugin = class extends import_obsidian2.Plugin {
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: (_editor, _view) => {
        this.getChatCbtRepsonse({ isSummary: false, mode: this.settings.mode });
      }
    });
    this.addCommand({
      id: "summarize",
  ...
  async getChatCbtRepsonse({ isSummary = false, mode = "openai" }) {
    const activeFile = this.app.workspace.getActiveFile();
    if (!activeFile) {
      return;
    }
    if (!VALID_MODES.includes(this.settings.mode)) {
      new import_obsidian2.Notice(`Inavlid mode '${this.settings.mode}' detected. Update in ChatCBT plugin settings and select a valid mode`);
      return;
    }
    ...
    let response = "";
    try {
      new import_obsidian2.Notice(`Asking ChatCBT... (${this.settings.mode} mode)`);
      ...
      new import_obsidian2.Notice(`ChatCBT failed :(: ${e.message}`);
      console.error(e);
    } finally {
      loadingModal.close();
    }
...
var MySettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("OpenAI API Key").setDesc("Create an OpenAI API Key from their website and paste here").addText(
      (text) => text.setPlaceholder("Enter your API Key").setValue(this.plugin.settings.openAiApiKey ? crypt.decrypt(this.plugin.settings.openAiApiKey) : "").onChange(async (value) => {
        if (!value.trim()) {
