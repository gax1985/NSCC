---
title: "Manpage of VSFTPD.CONF"
source: "https://security.appspot.com/vsftpd/vsftpd_conf.html"
author:
published:
created: 2025-11-10
description:
...
[Index](https://security.appspot.com/vsftpd/#index) [Return to Main Contents](http://localhost/cgi-bin/man/man2html)
...
If activated, you may provide a list of anonymous password e-mail responses which cause login to be denied. By default, the file containing this list is /etc/vsftpd.banned\_emails, but you may override this with the **banned\_email\_file** setting.
...
When enabled, all FTP requests and responses are logged, providing the option xferlog\_std\_format is not enabled. Useful for debugging.
...
If set to yes, all SSL data connections are required to exhibit SSL session reuse (which proves that they know the same master secret as the control channel). Although this is a secure default, it may break many FTP clients, so you may want to disable it. For a discussion of the consequences, see [http://scarybeastsecurity.blogspot.com/2009/02/vsftpd-210-released.html](http://scarybeastsecurity.blogspot.com/2009/02/vsftpd-210-released.html) (Added in v2.1.0).
...
If enabled, a log file will be maintained detailling uploads and downloads. By default, this file will be placed at /var/log/vsftpd.log, but this location may be overridden using the configuration setting **vsftpd\_log\_file**.
...
This options specifies a comma separated list of allowed FTP commands (post login. USER, PASS and QUIT and others are always allowed pre-login). Other commands are rejected. This is a powerful method of really locking down an FTP server. Example: cmds\_allowed=PASV,RETR,QUIT
...
This option is the name of the file to which we write the wu-ftpd style transfer log. The transfer log is only written if the option **xferlog\_enable** is set, along with **xferlog\_std\_format**. Alternatively, it is written if you have set the option **dual\_log\_enable**.

Default: /var/log/xferlog

## AUTHOR

[scarybeasts@gmail.com](https://security.appspot.com/vsftpd/)

---

## Index

[NAME](https://security.appspot.com/vsftpd/#lbAB)

[DESCRIPTION](https://security.appspot.com/vsftpd/#lbAC)

[FORMAT](https://security.appspot.com/vsftpd/#lbAD)

[BOOLEAN OPTIONS](https://security.appspot.com/vsftpd/#lbAE)

[NUMERIC OPTIONS](https://security.appspot.com/vsftpd/#lbAF)

[STRING OPTIONS](https://security.appspot.com/vsftpd/#lbAG)

[AUTHOR](https://security.appspot.com/vsftpd/#lbAH)

---

This document was created by [man2html](http://localhost/cgi-bin/man/man2html), using the manual pages.  
