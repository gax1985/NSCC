...
> [!note]
>
> Suppose an HTTP client makes a request to the www.example.com web server.  The client has never requested a given base object, nor has it communicated recently with the www.example.com server. You can assume, however, that the client host knows the IP address of www.example.com.

#### Question 1

How many round-trip mes (RTTs) are needed from when the client first makes the request to when the base page is completely downloaded, assuming the me needed by the server to transmit the base page into the server's link is equal to 1/2 RTT and that the me needed to transmit the HTTP GET into the client's link is zero?  (You should consider any TCP setup time required before the HTTP GET is sent by the client, the me needed for the server to transmit the
requested object, and any propaga on delays not accounted for in these amounts of time.)


##### Answer Process :


We have four types of delay that would contribute to the total delay : 
...
Thus, imagine a packet of length **$L$** traveling from **Host A** **$\rightarrow$** **Switch** **$\rightarrow$**  **Host B** .

* **Link 1 (A **$\rightarrow$** Switch):** Transmission rate **$R_1$**, Distance **$D_1$**.
* **Link 2 (Switch **$\rightarrow$** B):** Transmission rate **$R_2$**, Distance **$D_2$**.
* **Assumption:** The problem states that queuing and processing delays are negligible (zero).
...
1. t=0, the local host requests that the name dal.ca be resolved to an IP address.
3. t=1, the local host requests that the name icann.org be resolved to an IP address.
4. t=5, the local host requests that the name concordia.ca be resolved to an IP address. (Hint: be
   careful!)
5. t=10, the local host again requests that the name dal.ca be resolved to an IP address.
6. t=12, the local host requests that the name utoronto.ca be resolved to an IP address.
7. t=30, the local host again requests that the name dal.ca be resolved to an IP address. (Hint: be
   careful!)
