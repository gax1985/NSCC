...
1. Q1. How many round-trip mes (RTTs) are needed from when the client first makes the request to when the base page is completely downloaded, assuming the me needed by the server to transmit the base page into the server's link is equal to 1/2 RTT and that the me needed to transmit the HTTP GET into the client's link is zero?  (You should consider any TCP setup me required before the HTTP GET is sent by the client, the me needed for the server to transmit the requested object, and any propaga on delays not accounted for in these amounts of me.)
2. ---
